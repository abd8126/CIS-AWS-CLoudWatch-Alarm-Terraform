...
resource "aws_s3_bucket_logging" "S3SharedBucketLogging" {
  bucket = aws_s3_bucket.S3SharedBucket.id
  target_bucket = aws_s3_bucket.S3SharedBucket.id
  target_prefix = ""
}

resource "aws_s3_bucket_server_side_encryption_configuration" "S3SharedBucketEncryptionConfiguration" {
  bucket = aws_s3_bucket.S3SharedBucket.id
...
resource "aws_s3_bucket_versioning" "S3SharedBucketVersioning" {
  bucket = aws_s3_bucket.S3SharedBucket.id
...
resource "aws_s3_bucket_acl" "S3SharedBucketACL" {
  bucket = aws_s3_bucket.S3SharedBucket.id
  acl = "log-delivery-write"
}

resource "aws_s3_bucket_public_access_block" "S3SharedBucketPublicAccessBlock" {
  bucket = aws_s3_bucket.S3SharedBucket.id
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::s3-bucket-random-name-BRLiw/*"
            ],
            "Condition": {
                "StringLike": {
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::s3-bucket-random-name-BRLiw"
        }
    ]
}
)
...
module "SnsTopic1" {
  ...
  stack_name = "tf-cfn-stack-SnsTopic1-SDalR"
}
